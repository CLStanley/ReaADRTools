...
| `ReaADR_Core_Persistence.lua` | Reference-only / core persistence parity verified | Active workflows are native-routed. Canonical model serialization, session construction/replacement, revision handling and safety snapshot/restore contracts have direct native owners. Remaining reference value is limited to UI window-state helpers, script-registry compatibility, legacy-region adoption and event/logging details that should be audited separately. |
...
- **Canonical session serialization:** native `session_model.cpp` preserves the Lua record families (`session`, project metadata, timecode, state/dirty flags, scripts, characters, cues, tracks, regions and imports), deterministic field encoding, the cue-field allowlist and unknown-record compatibility needed by `adr_session_model_v1`.
- **Session construction/replacement:** native `session_builder.cpp` and `session_mutation.cpp` preserve manual-script fallback, stable character/track/region identities, cue-count bookkeeping, lane-derived cue/dialogue tracks, stale track/region replacement, retained script metadata, zero counts for removed scripts/characters and import-registry preservation.
- **Repository/revision/snapshot persistence:** native `SessionModelRepository` owns `adr_session_model_v1`, the compatibility session-ID index, monotonic revision bumping, safety snapshots and model-only restore. Restore advances from the greater of current/restored revision, matching the Lua anti-stale revision contract while additionally resynchronizing the session-ID index.
...
Native workflow/action registration now owns Record Cue, Cue Info, Set Cue Status, Dialogue Detection and Quick Actions. The legacy host only *opportunistically* removes historical Lua registrations when REAPER exposes `AddRemoveReaScript`; extension startup and native workflow ownership no longer depend on that compatibility API. The persistent runtime binds the four public Quick Action slots directly to `_ReaADRQuickAction1Native` through `_ReaADRQuickAction4Native`. Historical Lua files remain compatibility launchers/reference only and are no longer transient runtime owners.
